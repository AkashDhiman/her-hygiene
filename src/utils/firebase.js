...
export const addPeriodRegister = async (
  user,
  { height, weight, age, cycleLength, cycleTotal }
) => {
  //userinfo -- name and phone
  if (!user) return;
  console.log(user.data);
  // const yhat=Ml(userInfo);
  try {
    await db.collection("users").doc(`${user.data.uid}`).set(
      {
...
export const updateLogPeriod2 = async (user, { selection }) => {
  ...
  const past = await userRef
    .collection("log")
    .doc(`${startOfMonth(subMonths(selection.endDate, 1))}`)
    .get();
  const future = await userRef
    .collection("log")
    .doc(`${startOfMonth(addMonths(selection.endDate, 1))}`)
    .get();
  let cycleLength = Math.abs(
    differenceInDays(selection.startDate, selection.endDate)
  );
  let cycleTotal = 28;
  if (past.exists && past.data().startDate) {
    ...
    cycleTotal = Math.abs(
      differenceInDays(past.data().startDate.toDate(), selection.startDate)
    );
    console.log(
      cycleTotal,
      past.data().startDate.toDate(),
      selection.startDate
    );
  }
  if (future.exists && future.data().startDate) {
    ...
    await userRef
      ...
      .set(
        {
          cycleTotal: differenceInDays(
            future.data().startDate.toDate(),
            selection.startDate
          ),
        },
        { merge: true }
  ...
  const ans = await userRef
    .collection("log")
    .doc(`${startOfMonth(selection.endDate)}`);
  if (ans.exists) {
    ans.set(
      {
        ...selection,
